...
import { GenericItem } from '../models/ConvertWorkloadModel';
...
export async function callItemCreate<T>(
    ...
    try {
        ...
        console.error(`Failed creating item with name ${displayName}`);
        console.error(exception);
        console.error(exception.error?.message?.code);
...
/**
 * Calls the Convert endpoint of the workload API to double the result.
 * 
 * @param {WorkloadClientAPI} workloadClient - An instance of the WorkloadClientAPI.
 * @param {string} workspaceObjectId - The workspace object ID.
 * @param {string} itemObjectId - The item object ID.
 * @returns {Promise<{ ConvertedResult: string }>} A Promise that resolves to an object containing the converted result.
 */
export async function callConvertItemConvertOperation(workloadBEUrl: string, workloadClient: WorkloadClientAPI, workspaceObjectId: string, itemObjectId: string, lakehouse: GenericItem, inputDataFileName: string, templateFileName: string, convertServiceEndpoint: string): Promise<{ ConvertedResult: string }> {
    try {
        const accessToken: AccessToken = await callAuthAcquireAccessToken(workloadClient);
        const response: Response = await fetch(`${workloadBEUrl}/${workspaceObjectId}/${itemObjectId}/convert`, {
            method: `POST`,
            headers: {
                'Authorization': 'Bearer ' + accessToken.token,
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ lakehouse, inputDataFileName, templateFileName, convertServiceEndpoint }),
        });

        if (!response.ok) {
            // Handle non-successful responses here
            const errorMessage: string = await response.text();
            console.error(`Error calling Convert API: ${errorMessage}`);
            throw new Error(`Error calling Convert API: ${errorMessage}`);
        }

        const result: { ConvertedResult : string } = await response.json();
