...
import { callAuthAcquireAccessToken, callDialogClose, callItemCreate, callPageOpen } from "../../controller/ConvertWorkloadController";
import { GenericItem, CreateItemPayload } from "../../models/ConvertWorkloadModel";
...
export function CreateItemDialog({ workloadClient, isImmediateSave }: SaveAsDialogProps) {
    ...
    const convertItemDisplayName = "Convert Workload Item";
    const convertItemEditorPath = "/convert-workload-editor";
        ...
        if (isImmediateSave) {
            try {
                ...
                setIsSaveDisabled(true);
                 ...
                 createResult = await handleCreateConvertItem(pageContext.workspaceObjectId, displayName, description);
            } catch(error) {
                switch(error?.error) {
    ...
    const handleCreateConvertItem = async (
        ...
        try {
            ...
            await callPageOpen(convertWorkloadName, `${convertItemEditorPath}/${createdItem.id}`, workloadClient);
        } catch (createError) {
            // name is already in use
            if (createError.error?.message?.code === "PowerBIMetadataArtifactDisplayNameInUseException") {
                setValidationMessage(`${convertItemDisplayName} name is already in use.`);
            }
            // capacity does not support this item type 
            else if (createError.error?.message?.code === "UnknownArtifactType") {
                setValidationMessage(`Workspace capacity does not allow ${convertItemDisplayName} creation`);
            }
            else {
                setValidationMessage(`There was an error while trying to create a new ${convertItemDisplayName}`);
            }
    ...
    return (
        <Stack className="create-dialog">
            <Stack className="section">
                <h2>Create {convertItemDisplayName}</h2>
